...
def extract_info(history):
    ...
    for record in history:
        ...
        for target_id, radar_id in assignments.items():
...
radar_info = {radar_id: 2 for radar_id in all_radars}
...
def convert_to_gantt_data(history):
    ...
    for i, record in enumerate(history):
        ...
        for target_id, radar_id in assignments.items():
            ...
            if radar_id is not None:
                # 检查是否需要创建新的段
                if not segments or segments[-1]["radar_id"] != radar_id or segments[-1]["end_time"] != timestamp:
                    # 如果是新的雷达或者时间不连续，创建新段
                    segments.append({
                        "target_id": int(target_id),
                        "radar_id": radar_id,
                        "channel_id": 0,  # 简化处理，假设使用第一个通道
